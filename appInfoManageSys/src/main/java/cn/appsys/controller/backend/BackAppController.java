...
@Controller
@RequestMapping(value="/appManager/backend/app")
...
//
	public List<DataDictionary> getDataDictionaryList(String typeCode){
		...
		try {
			dataDictionaryList = dataDictionaryService.getDataDictionaryListForAll(typeCode);
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
...
//
	...
	public String updateStatus(@RequestParam("status") int status, @RequestParam("id") int id, HttpServletResponse response) throws IOException {
		PrintWriter out = response.getWriter();
		try {
			int count =  appInfoService.updateSatus(status,id);
			if (count<1){
				out.print("<script>alert('审核失败！');window.location.href ='/appManager/backend/app/list';</script>");
			}
		} catch (Exception e) {
			e.printStackTrace();
			out.print("<script>alert('审核失败！');window.location.href ='/appManager/backend/app/list';</script>");
		}
		return "redirect:/appManager/backend/app/list";
	}
//	/**
...
//	 * @return
//	 */
	@RequestMapping(value="/check/{aid}/{vid}",method=RequestMethod.GET)
	public String checkview(@PathVariable String aid,@PathVariable String vid,Model model){
		AppInfo appInfo = null;
		AppVersion appVersion = null;
		try {
			appInfo = appInfoService.getAppInfo(Integer.parseInt(aid),null);
			appVersion = appVersionService.getAppVersionListByUser(Integer.parseInt(vid));
		}catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
