...
function  loadValuesId(){
    $.ajax({
        type:"GET",//请求类型
        url:"loadValuesId.json",//请求的url
        data:{valueId:$("#valueId").val(),typeName:$("#typeName").val()},//请求参数
        dataType:"json",//ajax接口（请求url）返回的数据类型
        success:function(data){//data：返回数据（json对象）
            if (data=="true"){
                $("#valueIdForCheck").html("<span style=\"color: red\">该数据的id值已存在！</span>");
                return false;
            }else{
                $("#valueIdForCheck").html("<span style=\"color: green\">该数据的id值可以使用！</span>");
                return true;
            }
        },
        error:function(data){//当访问时候，404，500 等非200的错误状态码
...
$(function() {
    ...
    $("#valueId").blur(function () {
          if (loadValuesId()==false){
             $(this).focus();
           }
        });
});
